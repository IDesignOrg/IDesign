...
public class CartController {
	...
	public String goToCart(HttpSession session, Model model) {
		...
		List<Long> shopNos = cartEntities.stream().map(cart -> cart.getShopEntity().getShopNo())
		...
		List<Long> cartNos = cartEntities.stream().map(cart -> cart.getCNo()).collect(Collectors.toList());
...
//		System.out.println("shopEntities: " + shopEntities);
		...
		List<CouponMapEntity> validCoupons = coupons.stream().filter(couponMap -> couponMap.getUsedDate() == null)
				.collect(Collectors.toList());

		List<Long> couponNos = validCoupons.stream().map(coupon -> coupon.getCouponEntity().getCouponNo())
				.collect(Collectors.toList());
